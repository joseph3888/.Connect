...
export function PostDetail() {
  ...
  useEffect(() => {
    setPost(dataService.getPostById(id));
  }, [id]);

  const handleLike = () => {
    if (user && post) {
      dataService.toggleLikePost(post.id, user.id || user.email);
      setPost(dataService.getPostById(Number(id)));
    }
  };

  const handleComment = () => {
    if (commentText.trim() && user && post) {
      dataService.addComment(post.id, user.name, commentText);
      setPost(dataService.getPostById(Number(id)));
      setCommentText('');
    } else if (!user) {
  ...
  const isLiked = Array.isArray(post.likes) && user ? post.likes.includes(user.id || user.email) : false;
